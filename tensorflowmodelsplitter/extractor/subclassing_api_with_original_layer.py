...
from tensorflowmodelsplitter.model_architecture_kinds.subclassing_api_with_original_layer import (
    SubclassingWithLayerModel,
)


# Same as Functional API.
def extract_layer(model: Model) -> Model:
    input_layer = model.get_layer(name="dense_1").input
...
def extract() -> np.ndarray:
    model = SubclassingWithLayerModel()
